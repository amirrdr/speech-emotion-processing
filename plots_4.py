...
lbl = 'Accuracy, Using ' + str(round(sizee[plts[i]] * 100)) + '% of Data'
plt.plot(iters, gaussian_filter1d(cls_emp[:, plts], sigma=sigma), label=lbl)
plt.plot(iters, gaussian_filter1d(cls_gen[:, plts], sigma=sigma), label=lbl)
plt.plot(iters, gaussian_filter1d(cls_gen_emo[:, plts], sigma=sigma), label=lbl)
