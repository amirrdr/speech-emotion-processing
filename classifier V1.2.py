...
ltn = Conv2D(50, 20, activation="relu", kernel_regularizer=regularizers.l2(0.001), padding="same")(ltn)
ltn = Dropout(0.2)(ltn)
ltn = Conv2D(100, 40, activation="relu", kernel_regularizer=regularizers.l2(0.001), padding="same", strides=(2, 2))(ltn)
ltn = Dropout(0.2)(ltn)
ltn = Conv2D(200, 80, activation="relu", kernel_regularizer=regularizers.l2(0.001), padding="same", strides=(4, 4))(ltn)
ltn = Dropout(0.2)(ltn)
ltn = BatchNormalization()(ltn)
ltn = GlobalAveragePooling2D()(ltn)
ltn = Dense(200, activation="relu")(ltn)
...
for i in range(9):
    ...
    log_dir = "./logs/cls/speaker_gender_emotion/" + str(round(100*sizee[i]))
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
    drct = './logs/cls/speaker_gender_emotion/cls_spk_gen_emo_' + str(round(100*sizee[i])) + '.csv'
